...
describe('AuthService', () => {
  ...
  describe('generateNonce', () => {
    ...
    it('should throw AuthError for invalid wallet address', async () => {
      ...
      await expect(authService.generateNonce(invalidAddress)).rejects.toMatchObject({
        code: 'INVALID_WALLET',
        statusCode: 400,
      });
    });
  ...
  describe('authenticateWithWallet', () => {
    it('should authenticate existing user with valid signature', async () => {
      ...
      const signature = 'valid-base64-signature';
    ...
    it('should throw error when message does not match stored nonce', async () => {
      ...
      await expect(
        authService.authenticateWithWallet(walletAddress, signature, differentMessage)
      ).rejects.toThrow(AuthError);
      await expect(
        authService.authenticateWithWallet(walletAddress, signature, differentMessage)
      ).rejects.toMatchObject({
  ...
  describe('verifyAccessToken', () => {
    ...
    it('should throw error for invalid token', () => {
      ...
      expect(() => authService.verifyAccessToken(invalidToken)).toThrow(AuthError);
    });

    it('should throw error for expired token', () => {
      const jwt = require('jsonwebtoken');
      const expiredToken = jwt.sign(
        { userId: 'test', walletAddress: 'test', role: 'INVESTOR', kycStatus: 'PENDING' },
        'test-jwt-secret-key-for-testing-purposes',
  ...
  describe('authenticateWithPassword', () => {
    it('should authenticate user with valid email and password', async () => {
      const testUser = createTestUser({
        ...
        passwordHash: '$2a$10$N9qo8uLOickgx2ZMRZoMye.3kSxoloNq.pmXxSVdWqHLe8rqWPVee', // password: "password123"
        role: 'PLATFORM_ADMIN',
        kycStatus: 'VERIFIED',
      });
    ...
    it('should throw error when user has no password', async () => {
      ...
      await expect(
        authService.authenticateWithPassword('wallet-user@example.com', 'password')
      ).rejects.toThrow(AuthError);
      await expect(
        authService.authenticateWithPassword('wallet-user@example.com', 'password')
      ).rejects.toMatchObject({
